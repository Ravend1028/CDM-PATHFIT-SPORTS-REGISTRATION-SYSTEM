<?php
  ...
  if (isset($_POST['reg-id'])) {
      ...
      $sql = "SELECT accounts.fullname, events.title, reg_list.date_time, reg_list.id
              ...
              WHERE reg_list.event_id = $registrationID AND reg_list.reg_status = 2";
  ...
  <div id="reglist-container" class="container">
    <?php if (!empty($registers)): ?>
        ...
        <div class="container">
            <div class="row">
                <div class="col p-3">
                    <h2 class="text-center">Rejected for <?php echo $registers[0]['title']; ?></h2>
                </div>
            </div>
        </div>
    ...
    <?php else: ?>
        ...
        <p class='lead mt-3 text-center'>No one has been rejected yet.</p>
    <?php endif; ?>  
  </div>